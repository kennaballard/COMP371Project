...
protected:
    ...
    void DrawModel(Project::DrawContext context) {
        ...
        GLfloat shiftY = 0.0f;
        // Back
        transformCube(shader, -2.0f + shiftX, shiftY, defaultSize, 1.0f, defaultSize);
        glDrawElements(GL_LINE_LOOP, 36, GL_UNSIGNED_INT, 0);

        // Bridge
        transformCube(shader, shiftX, shiftY, defaultSize, defaultSize, defaultSize);
        glDrawElements(GL_LINE_LOOP, 36, GL_UNSIGNED_INT, 0);

        GLfloat tailHeight = (1 - defaultSize) / 2;
        // Upper tail
        transformCube(shader, 2.0f + shiftX, 1 + (defaultSize / tailHeight) + shiftY, defaultSize, tailHeight, defaultSize);
        glDrawElements(GL_LINE_LOOP, 36, GL_UNSIGNED_INT, 0);
